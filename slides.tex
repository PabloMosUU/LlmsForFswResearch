% handout/final = bullet points etc. one by one (final) or all on the slide immediately (handout)
\documentclass[t,xcolor={dvipsnames},final,aspectratio=169]{beamer}
\beamertemplatenavigationsymbolsempty
\usepackage{adjustbox}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{url}
\usepackage{hyperref}
\usepackage{tikz}
\usepackage{smartdiagram}
\usepackage{booktabs}
\setbeameroption{show notes on second screen}
%\setbeameroption{hide notes}

\mode<presentation> {
    \usetheme{Frankfurt}
    \usecolortheme{crane}
    \setbeamertemplate{footline}[frame number]
    \beamerdefaultoverlayspecification{<+->}
}

\title{How to handle LLMs in your research}
\author{Pablo Mosteiro, Robert Bagheri, \"Ozg\"ur Togay, Laurence Frank\\
\tiny{Partly generated with help of Qwen/Qwen2.5-7B-Instruct and moonshotai/Kimi-K2-Instruct-0905 via together and groq, respectively (HuggingChat).}}
\institute{Department of Methodology, Statistics \& Data Science}
\date{\today}
\logo{\includegraphics[scale=0.5]{img/UU-logo2011_CMYK-eps-converted-to.pdf}\hspace*{0.75\textwidth}\vspace{-8mm}}

\begin{document}

\begin{frame}
    \maketitle
\end{frame}

\begin{frame}{Today's Agenda}
    \begin{itemize}
        \item Introduction (3 min)
        \item How LLMs Work (10 min)
        \item Limitations and Risks (10 min)
        \item Prompting CAs and Chatbots (10 min)
        \item Practical LLM Use in Research (10 min)
        \item Conclusion (2 min)
    \end{itemize}
\end{frame}

\section{Intro}

{\setbeamertemplate{logo}{}
\begin{frame}{About us}
\begin{center}
\includegraphics[width=0.5\textwidth]{img/nltp.png}

\url{https://nlp.sites.uu.nl/}
\end{center}
\end{frame}
}

\begin{frame}{Introduction: What do you know about LLMs?}
\begin{itemize}
\item    \textbf{Activity: Think-Pair-Share} (2 min)
    \end{itemize}
\includegraphics[width=0.25\textwidth]{img/dalle_ai.png}
\tiny Image produced in DALL-E December 2023 (Credit: Christine Fox)
\end{frame}

\section{How LLMs work}

\begin{frame}{AI vs ML vs LLM: The Relationship}
    \begin{center}
    \begin{tikzpicture}[scale=0.8]
        \draw[fill=blue!20] (0,0) circle (3cm);
        \node at (0,2.5) {\textbf{Artificial Intelligence}};
        
        \draw[fill=green!20] (0,0) circle (2cm);
        \node at (0,1.5) {\textbf{Machine Learning}};
        
        \draw[fill=red!20] (0,0) circle (1cm);
        \node at (0,0) {\textbf{LLMs}};
        
        \node[align=center] at (0,-4) {LLMs are a specific type of ML,\\ which is a subset of AI};
    \end{tikzpicture}
    \end{center}
    \note{
    Artificial Intelligence, ML, and LLMs: Key Definitions \\
    \textbf{Artificial Intelligence (AI)} \\
        Computer systems performing tasks requiring human intelligence\\
        Examples: Chess playing, image recognition, language translation\\
    
    \textbf{Machine Learning (ML)}\\
        Subset of AI using algorithms that improve through experience \\
        Learns patterns from data without explicit programming \\
    
    \textbf{Large Language Models (LLMs)}\\
        AI systems trained on vast text amounts \\
        Understand and generate human language \\
        Examples: GPT, Claude, Qwen, Mistral \\
}
\end{frame}

\begin{frame}{Model Learning Process: Training Phase}
    \textbf{Training Phase}
    \begin{itemize}
        \item Model learns patterns from large datasets
        \item Adjusts model to minimize prediction errors
        \item Requires massive computational resources (weeks / months)
    \end{itemize}
    
    \textbf{Example}: LLM learns that "cat" often appears with "dog" in pet-related contexts
\end{frame}

\begin{frame}{Model Learning Process: Validation Phase}
    \textbf{Validation and Testing}
    \begin{center}
\includegraphics[width=0.5\textwidth]{img/train_val_test.png}

\tiny Source: \url{https://dqlab.id/5-panduan-praktis-membuat-model-machine-learning}
\end{center}
\note{
During validation:\\
    \begin{itemize}
        \item Test model on new, unseen data
        \item Measure performance using metrics (accuracy, precision, recall)
        \item Prevent memorizing training data
        \item Tune the model for optimal performance
    \end{itemize}
    }
    
    \textbf{Key insight}: Good models \textbf{generalize}, not just \textbf{memorize}
\end{frame}

\begin{frame}{Prompting phase}
\centering
\Large

\only<1->{U}
\only<2->{ weet}
\only<3->{ wat}
\only<4->{ ik}
\only<5->{ ga}

\vspace{1cm}

\only<1>{\includegraphics[width=.6\textwidth]{img/soup_u.png}}
\only<2>{\includegraphics[width=.6\textwidth]{img/soup_weet.png}}
\only<3>{\includegraphics[width=.6\textwidth]{img/soup_wat.png}}
\only<4>{\includegraphics[width=.6\textwidth]{img/soup_ik.png}}
\only<5>{\includegraphics[width=.6\textwidth]{img/soup_ga.png}}

\vfill

\scriptsize
Credit for this slide: Albert Gatt \& Dong Nguyen

\end{frame}

\begin{frame}{What is Generative AI?}
    \textbf{Definition}: AI that creates new content (text, images, code, audio)
    
    \textbf{Key Features}:
    \begin{itemize}
        \item Creates novel outputs, not just classifies
        \item Learns patterns from training data
        \item Generates human-like content
    \end{itemize}
    
    \textbf{Examples}: ChatGPT (text), DALL-E (images), CodeT5 (code)
    \note{
    LLMs are a form of GenAI (they are generative after all)
    }
\end{frame}

\begin{frame}{Text-GenAI components}
\small
\begin{tabular}{clll}
& \textcolor{orange}{\textbf{consists of}} & \textbf{fed by} & \textbf{GPT learns} \\
\includegraphics[width=0.05\textwidth]{img/graph.png} & \textbf{LLM} & textual internet data & ``knowledge'' of language and content \\[0.6em]
\includegraphics[width=0.05\textwidth]{img/graph.png} & \textbf{Dialogue Model} & social media, forums & ``knowledge'' of dialogue structure \\[0.6em]
\includegraphics[width=0.05\textwidth]{img/person.png} & \textbf{Task Model} & programmers & tasks, self knowledge and limits \\[0.6em]
\includegraphics[width=0.05\textwidth]{img/person.png} & \textbf{Ethical Governor} & Kenian data labellers & ethical answers \\[0.6em]
\end{tabular}

\tiny Credit for this slide: Davitze K\"onning
\note{
``Knowledge'' b/c chatbots have no real knowledge or understanding; they produce what we want to hear based on predictions. \\
As a result, chatbots are better at being linguistically correct than at being content-wise correct.

The programmers also play a major role. You notice that adjustments are being made. \\
If you ask certain questions, the chatbots no longer want to answer them. \\
For example, there were many errors regarding the generation of incorrect sources.

LLM = Self-supervised learning\\
DM = learns how people interact with each other through social media, conversation data to make ChatGPT more human\\
TM = How to handle specific tasks, such as writing code, making plans, etc\\
EG = Filters, guidelines, and human supervision

Kenyan data labelers: come back to this point later
}
\end{frame}


\begin{frame}{Generative AI vs. Other Models}
    \begin{center}
    \begin{tabular}{lcc}
    \toprule
    \textbf{Feature} & \textbf{Generative} & \textbf{Discriminative} \\
    \midrule
    Primary Task & Create new content & Classify existing data \\
    Output Type & Novel text/images & Labels/categories \\
    Example & GPT, DALL-E & Spam filters, medical diagnosis \\
    \bottomrule
    \end{tabular}
    \end{center}
\end{frame}

\begin{frame}{Activity: True or False?}
    \textbf{Instructions}: Raise your hand if you think the statement is TRUE
    \begin{itemize}
\item     \textbf{Statement 1}: Generative AI can only work with text
    
    \item \textbf{Statement 2}: All generative models are also discriminative\begin{alertblock}{Caveat}
gold labels!
\end{alertblock}
    
    \item \textbf{Statement 3}: Generative AI creates completely original content never seen in training
    
    \item \textbf{Statement 4}: LLMs are a type of generative AI
    \end{itemize}
\end{frame}

\section{Limitations and Risks}

\begin{frame}{Limitations and Risks: Hallucinations}
    \textbf{What are hallucinations?}
    \begin{itemize}
        \item LLMs generate false or nonsensical information (fake citations, non-existent facts)
        \item Appears confident and plausible
    \end{itemize}
\includegraphics[width=0.25\textwidth]{img/hallucination.png}
    \note{
    \textbf{Example}: Asking for ``recent studies on X'' might return made-up citations\\    
    \textbf{Risk}: Researchers might unknowingly use false information
    }
\end{frame}

\begin{frame}{Bias}
    \begin{itemize}
        \item \textbf{Gender bias:} Assumes doctors are male, nurses are female
        \item \textbf{Racial bias:} Associates certain groups with negative stereotypes
        \item \textbf{Cultural bias:} Primarily represents Western perspectives
        \item \textbf{Academic bias:} Favors published, English-language sources
        \item \textbf{Confirmation bias:} Reinforces existing beliefs
        \item \textbf{Temporal bias:} Becomes outdated as language/society changes
    \end{itemize}
    
    \textbf{Real-world impact}: Biased models can perpetuate harmful stereotypes
    \note{Spend some time on each of the types of bias. Make sure they are clear.\\
    
    Caused by input data or model architecture.}
\end{frame}

{\setbeamertemplate{logo}{}
\begin{frame}{Limitations and Risks: Output Reliability}
    \textbf{Reliability Issues}
    \begin{itemize}
        \item \textbf{Inconsistency:} Same prompt can yield different answers
        \only<1>{
        \begin{figure}
            \centering
            \includegraphics[width=0.9\textwidth]{img/life1.png}
            \includegraphics[width=0.9\textwidth]{img/life2.png}
        \end{figure}
        }
        \item \textbf{Context dependency:} Performance varies by domain
        \only<2>{
        \begin{figure}
            \centering
            \includegraphics[width=0.5\textwidth]{img/susurro.png}
        \end{figure}
        }
        \item \textbf{Temporal drift:} Knowledge becomes outdated
        \only<3>{
        \begin{figure}
            \centering
\includegraphics[width=0.5\textwidth]{img/wordboundaries.png}
\includegraphics[width=0.4\textwidth]{img/westgermanic.png}
        \end{figure}
  }
        \item \textbf{Prompt sensitivity:} Small changes affect outputs significantly
        \only<4>{
        \begin{figure}
            \centering
            \includegraphics[width=0.45\textwidth]{img/basic.png}
            \includegraphics[width=0.45\textwidth]{img/verybasic.png}
        \end{figure}
        }
    \end{itemize}

    \textbf{Best practice}: Always verify critical information from multiple sources
\end{frame}
}

{\setbeamertemplate{logo}{}
\begin{frame}{Limitations and Risks: Ethics}
    \textbf{Ethical Concerns}
    \begin{itemize}
        \item \textbf{Data:} Training data may include personal or copyrighted material
        \item \textbf{Environmental impact:} High energy consumption
        \item \textbf{Labor practices:} Data annotation often involves poor working conditions
        \only<3>{\hfill\includegraphics[height=4cm]{img/labor.png}\\
        \footnotesize{\url{https://www.cbsnews.com/news/ai-work-kenya-exploitation-60-minutes} (Nov 2024)}
}  % visible only on slide 4
        \item \textbf{Academic integrity:} attribution of AI assistance / uploading restricted data
        \item \textbf{Open source:} Often little documentation on data, training, architecture\\
        \only<5>{``Given both the competitive landscape and the safety implications of large-scale models like GPT-4, this report contains no further details about the architecture (including model size), hardware, training compute, dataset construction, training method, or similar''

OpenAI: Achiam, J. et al (2024). \textit{GPT-4 Technical Report}. \url{https://arxiv.org/abs/2303.08774}}
\item \textbf{Effect on cognition:}  
\includegraphics[width=0.6\textwidth]{img/cognition.png}\\
\footnotesize{Dubey et al. Redefining Cognitive Domains in the Era of ChatGPT: A Comprehensive Analysis of Artificial Intelligence's Influence and Future Implications. Med Res Arch. 2024 Jun;12(6):5383}
    \end{itemize}
\end{frame}
}

\section{Prompting}
{\setbeamertemplate{logo}{}
\begin{frame}{Effective Prompting: BRAVE(R) Framework}
  \begin{columns}
    \begin{column}{0.45\textwidth}
    \begin{center}
\includegraphics[width=0.9\textwidth]{img/braver.png}
\end{center}
    \end{column}
    \begin{column}{0.45\textwidth}  %%<--- here
    \textbf{Poor prompt}: ``Summarize this paper''
    
    \textbf{Better prompt}: ``Act as a psychology research assistant [R]. Summarize this paper in 3 bullet points [B] focusing on methodology and findings relevant to cognitive behavioral therapy [V]. Use academic language [A] and APA style [E].''
    
    \textbf{Key insight}: Specific, contextual prompts yield better results
    \end{column}
  \end{columns}
\end{frame}
}

\begin{frame}{Evaluating and Critically Assessing GenAI Responses}
\includegraphics[width=8cm]{img/facts.png}
More info: \url{https://teachersguidegsls.nl/genai/generative-ai-guidelines/}
\end{frame}

{\setbeamertemplate{logo}{}
\begin{frame}{Ways to prompt}
\includegraphics[width=0.9\textwidth]{img/huggingchat.png}
\end{frame}
}

{\setbeamertemplate{logo}{}
\begin{frame}{Switch models}
\includegraphics[width=0.9\textwidth]{img/models.png}
\note{
Using an LLM locally (no server) \\
But model can still be train in a very closed-source way \\
}
\end{frame}
}

\section{Practical LLM Use in Research}
\begin{frame}{Official UU policy}
\url{https://www.uu.nl/en/organisation/ai-policy/researchers}
\begin{itemize}
\item Benefits and use cases
\item Risks and limitations
\item Effective use of GenAI (\emph{Background check})
\item Citing GenAI in your work
\end{itemize}
\note{I don't want to go into details because this changes all the time.}
\end{frame}

\begin{frame}{EduGenAI}
\url{https://www.uu.nl/en/organisation/ai-policy/teachers/edugenai}
\includegraphics[width=0.5\textwidth]{img/edugenai.png}
\end{frame}

\begin{frame}{UU AI-chat}
\url{https://www.uu.nl/en/organisation/ai-policy/ai-platforms-at-uu} \\
\hspace{0.4\textwidth} \includegraphics[width=0.5\textwidth]{img/uuaichat.png}
\end{frame}

\begin{frame}{LLM Use in Research}
    \textbf{Appropriate \& Inappropriate Uses}
    \begin{itemize}
        \item Think-Pair-Share
    \end{itemize}
    \end{frame}

\begin{frame}{LLM Use in Research: Appropriate Applications}
    \textbf{Appropriate Uses}
    \begin{itemize}
        \item \textbf{Literature review:} Summarizing papers (with verification)
        \item \textbf{Writing assistance:} Drafting, editing, proofreading
        \item \textbf{Code generation:} Writing analysis scripts \begin{alertblock}{Caveat}
\textbf{only} when you know which statistical analysis is suitable
\end{alertblock}
        \item \textbf{Brainstorming:} Generating research ideas
        \item \textbf{Translation:} Working with multilingual sources
    \end{itemize}
    
    \textbf{Key}: Always maintain human oversight and verification
\end{frame}

\begin{frame}{LLM Use in Research: Inappropriate Applications}
    \textbf{Avoid Using LLMs For:}
    \begin{itemize}
        \item \textbf{Statistical analysis:} LLMs can't perform proper statistical tests
        \item \textbf{Fact-checking:} They may hallucinate information
        \item \textbf{Critical decisions:} Participant safety, ethical approvals
        \item \textbf{Original thinking:} Generating truly novel theories
        \item \textbf{Replacing expertise:} They supplement but don't replace researcher knowledge
    \end{itemize}
\end{frame}

{\setbeamertemplate{logo}{}
\begin{frame}{Working with others}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
\includegraphics[width=0.75\textwidth]{img/ghost.png}
    \end{column}
    \begin{column}{0.45\textwidth}  %%<--- here
    \begin{enumerate}
    \item thinking for yourself
    \item read literature
    \item ask AI for advice
    \item critically analyze
    \end{enumerate}
    \end{column}
  \end{columns}
\note{
What can go wrong?

Encourage student assistants, PhD candidates and postdocs to triple-check outputs of GenAI. GenAI's coding skills are great, but if we don't scrutinize what it does, we don't understand the intermediate steps, and it becomes harder to make choices. Especially when choosing methodologies.
}
\end{frame}
}

\section{Conclusion}

\begin{frame}{Key Takeaways: LLM Workflow}
    \textbf{Remember:}
    \begin{itemize}
        \item LLMs are powerful tools, not replacements for human expertise
        \item Always prioritize ethics, privacy, and transparency
        \item Verify outputs and maintain critical thinking
    \end{itemize}
\end{frame}

\end{document}